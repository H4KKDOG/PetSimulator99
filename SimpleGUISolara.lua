...
local function loadSettings()
    if isfile("ps99settings.json") then
        local settings = HttpService:JSONDecode(readfile("ps99settings.json"))
...
local function createToggleGui(parent, buttonText, onClick)
...
if UserInputService.TouchEnabled then
    local toggleButton = createToggleButton(ScreenGui, "Toggle GUI", function()
...
local function handleKeybind(input)
    if input.UserInputType == Enum.UserInputType.Keyboard then
        ...
        if not focusedTextBox and input.KeyCode == Enum.KeyCode[getgenv().Keybind] then
            toggleGuiVisibility()
        end
    end
end
...
local function XZDist(obj1, obj2)
    return (Vector2.new(obj1.Position.X, obj1.Position.Z) - Vector2.new(obj2.Position.X, obj2.Position.Z)).Magnitude
end

local function findNearestBreakable()
    local nearestBreakable, nearestDistance = nil, 250
    for _, breakable in ipairs(Breakables:GetChildren()) do
        local meshPart = breakable:FindFirstChildWhichIsA("MeshPart")
        if meshPart then
            local distance = XZDist(HumanoidRootPart, meshPart)
...
local function isBreakableInRadius(breakable)
    ...
    return hitbox and XZDist(hitbox, HumanoidRootPart) <= 150
end
...
local function findNearestEgg()
    local nearestEgg, nearestDistance = nil, 250
...
local function eggnameHatch()
    if getgenv().AutoHatchNormal then
...
print("AutoHatchEvent is set to hatch nearest so dont need update when event or new custom egg")
print("AutoHatchNormal is for normal world or area eggs")
print("AutoHatchNormal only works in world egg or the one thats always in area/worlds")
print(" ")
